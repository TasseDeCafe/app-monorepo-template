...
const Footer = async ({ lang }: LangProps) => {
  return (
    <footer className='relative flex w-full flex-col items-center gap-y-4 bg-black p-12 text-center text-white'>
      ...
      <a href='mailto:contact@yourbestaccent.com' className='flex items-center gap-x-2 hover:underline'>
        <Image src='/images/icons/lucide/email.svg' alt='yourbestaccent logo' width={20} height={20} priority />
        contact@yourbestaccent.com
      </a>
      <div className='mt-2'>© {new Date().getFullYear()} YourBestAccent.com</div>
