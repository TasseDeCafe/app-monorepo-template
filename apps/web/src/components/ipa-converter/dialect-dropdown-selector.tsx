...
type DialectSelectorProps = {
  value: DialectCode | undefined
  onDialectSelect: (value: DialectCode) => void
  dialectCodes: readonly DialectCode[]
  placeholder?: string
  className?: string
}

export const DialectDropdownSelector = ({
  value,
  onDialectSelect,
  dialectCodes,
  placeholder,
  ...
  const placeholderText = placeholder ?? t`Select dialect`

  const dialectOptions = dialectCodes.map((dialectCode: DialectCode) => ({
    label: i18n._(dialectNameMessages[dialectCode]),
    value: dialectCode,
  }))
    ...
    <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen} modal={true}>
      <PopoverTrigger asChild>
        ...
        >
          <div className='flex flex-row items-center gap-2'>
            ...
            {value ? dialectOptions.find((option) => option.value === value)?.label : placeholderText}
          </div>
      ...
      <PopoverContent className='w-[--radix-popover-trigger-width] p-0' align='start'>
        <Command className='bg-white'>
          <CommandInput placeholder={t`Search dialect...`} className='h-9' />
          <CommandEmpty>{t`No dialect found.`}</CommandEmpty>
          <CommandList className='max-h-[200px]'>
            <CommandGroup>
              {dialectOptions.map(({ label, value: optionValue }) => (
                <CommandItem
                  className={cn('cursor-pointer px-3 py-2 hover:bg-gray-100')}
                  value={label}
                  key={optionValue}
                  onSelect={() => {
                    onDialectSelect(optionValue)
